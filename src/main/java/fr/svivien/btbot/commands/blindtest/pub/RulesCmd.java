...
public class RulesCmd extends BTPublicCommand {
    ...
    public void doCommand(CommandEvent event) {
        String rules = ":bulb: **Submissions :** :bulb:\n" +
                ...
                "* In case they are not, you can update them manually using the commands `" + prefix + "set <song index> <value name> <value>`\n" +
                "* Players can also add/remove up to " + maximumExtras + " extra input(s) to guess using the command `" + prefix + "set <song index> <value name> <value>` and `" + prefix + "unset <song index> <value name>`\n" +
                "\n" +
                ...
                "* 1 point is awarded each time a player finds a value that wasn't previously guessed. Each subsequent good answer by the same player on the same track will reward 2 points\n" +
